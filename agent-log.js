...
function createAgentLog({ socket = '' } = {}) {
  ...
  function tmuxInfo(target) {
    return new Promise((resolve) => {
      const args = [...(socket ? ['-L', socket] : []), 'display-message', '-p', '-t', target, '#{pane_current_path}\t#{pane_current_command}'];
      execFile('tmux', args, { timeout: 3000 }, (err, out) => {
        if (err) return resolve(null);
        const [cwd, cmd] = String(out).trim().split('\t');
        resolve({ cwd, cmd });
      });
    });
  }
  ...
  async function resolve(target) {
    ...
    let file = null;
    if (info && /claude/i.test(info.cmd || '') && info.cwd) {
    ...
    const r = { file, cwd: info && info.cwd, at: Date.now() };
    where.set(target, r);
    return r;
  }

  function state(file) {
    if (!logs.has(file)) logs.set(file, { offset: 0, rev: 0, items: [], byId: new Map(), tools: new Map(), title: '', session: path.basename(file, '.jsonl'), carry: '' });
  ...
  function update(file) {
    ...
    if (size < st.offset) { logs.delete(file); return update(file); }
    ...
    try {
      ...
      for (const line of lines) {
        ...
        if (line.trim()) { let j; try { j = JSON.parse(line); } catch { j = null; } if (j) ingest(st, j, pos, len); }
        pos += len;
      }
      st.offset = size;
  ...
  async function read(target, since, limit = 250) {
    ...
    const st = update(r.file);
    const items = since > 0 ? st.items.filter((x) => x.rev > since) : st.items.slice(-limit);
    return { available: true, session: st.session, title: st.title, model: st.model || '', modelName: modelName(st.model), cwd: r.cwd, rev: st.rev, reset: since > st.rev, items };
  }
...
module.exports = { createAgentLog, toolLabel, cleanUser, modelName };
